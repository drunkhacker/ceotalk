class CompaniesController < ApplicationController
  ...
  def index
    ...
    if @name_filter >= 0 and @name_filter < @indexes.length
      @companies = Company.where("name >= ? AND name <= ?", offsets[@name_filter*2], offsets[@name_filter*2+1]).page(params[:page]).per(20)
    elsif @name_filter < 0
      @companies = Company.page(params[:page]).per(20)
    else #기타
      @companies = Company.where("name < ? OR name > ?", offsets.first, offsets.last).page(params[:page]).per(20)
    end
  end
  ...
  def show
    ...
    @top_comments = @company.comments.where("like_count >= 3").order("like_count DESC").limit(3)
